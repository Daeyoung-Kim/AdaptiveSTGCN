...
for i in range(len(Errors)) :
  try :
    pd.read_csv(folder + '/' + Errors[i], encoding='utf-8')
    raw_data = pd.read_csv(folder + '/' + Errors[i], encoding='utf-8').reset_index()
    raw_data.columns = ['사용일자', '노선명', '역명', '승차총승객수', '하차총승객수', '등록일자', 'NaN']
    raw_data = raw_data.drop(columns = [ 'NaN'])
    raw_data = raw_data.set_index('사용일자')
    Raw.append(raw_data)
...
# 중복된 데이터 최종 처리
# data_filtered : OO역, OO역(OO산) 과 같은 데이터 전부 제거된 데이터프레임
# toreplace : 위에서 제거된 OO역에 대한 정식 데이터만을 담은 데이터프레임
filtered = sorted(list(set(filter1) | set(filter2))) # 중복되는 쌍 리스트
data_index_list = list(data.reset_index().set_index('역명').index) # 원본데이터 인덱스 리스트
intersection = sorted(list(set(filtered) & set(data_index_list))) # 교집합
data_filtered = data.drop(intersection)

final = pd.concat([data_filtered, toreplace], axis=0, join='outer')
final_header_ascending = sorted(list(final.columns))
final = final[final_header_ascending]
#final.to_csv('./Data/StationTotalxxxxxx.csv')