...
class AdapBlock(nn.Module):
    # AdapBlock contains 'GND' structure
    # G: Graph Convolution Layer (ChebGraphConv or GraphConv)
    # N: Layer Normolization
    # D: Dropout
    
    def __init__(self, Ks, n_vertex, last_block_channel, channel, graph_conv_type, bias, droprate):
        super(AdapBlock, self).__init__()
        self.graph_conv = GraphConvLayer(graph_conv_type, last_block_channel, channel, Ks, bias)
        self.tc2_ln = nn.LayerNorm([n_vertex, channel])
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(p=droprate)

    def forward(self, x, adp_gso):
        x = self.graph_conv(x, adp_gso)
        x = self.relu(x)
        x = self.tc2_ln(x.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        x = self.dropout(x)
